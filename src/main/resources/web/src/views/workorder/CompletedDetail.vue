...
const fetchOrderDetail = async (id: string) => {
  ...
  headers: {
    ...
    if (response.code === 200) {
      ...
      // 根据后端 WorkOrder 实体结构处理数据
      orderDetail.value = {
        id: data.orderId || '',
        orderNo: data.orderId || '',
        status: data.status as OrderStatus || 'completed',
        repairman: data.assignedRepairmanId || '未知维修员', // 需要根据维修员ID获取姓名
        phone: '1234567890', // 需要从维修员信息中获取
        ...
        totalCost: '0元', // 需要根据实际业务逻辑计算
        reviewOpinion: '审核通过', // 需要根据实际审核信息获取
